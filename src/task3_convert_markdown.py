...
def pdf_markdown(path):
    """Keep page anchors and table boundaries rather than interleaving columns."""
    pages = []
        ...
        for index, page in enumerate(document.pages, 1):
            ...
            for table in tables:
                ...
                for row in table.extract():
                    # Some PDFs split a single cell into several physical rows.
...
def render(path, metadata, body):
    ...
    for key in ('metadata_note', 'source_verification', 'source_verified_on',
                'content_start_line', 'content_end_line', 'normalization'):
        if key in metadata:
...
def article_body(markdown):
    """Select the article body from the two page templates in this archive.

    Retain a contiguous source span and record 1-based line bounds for debugging.
    Fail on unknown templates instead of silently indexing navigation.
    """
    ...
    start += 1
    # UET has author, publication date, category and view count below its H1.
    if start < len(lines) and '/author/' in lines[start]:
        while start < len(lines) and (lines[start].startswith('  * ') or not lines[start].strip()):
            start += 1
    endings = ('[ ![hình đại diện tác giả]', '[Bài trước]', '[Bài tiếp',
               '### Bài viết liên quan', 'Xem thêm:', '### Bài viết cùng chủ đề:')
...
def convert_news_articles():
    ...
    for path in sorted((LANDING_DIR / 'news').glob('*.json')):
        ...
        data.update(content_start_line=start, content_end_line=end,
...
def convert_all():
    ...
    expected = {OUTPUT_DIR / path.relative_to(LANDING_DIR).with_suffix('.md')
                for path in (ROOT / name for name in records)}
    ...
    write_json(ROOT / 'data/corpus_manifest.json', {
        'source_archive': ARCHIVE.name,
        'archive_sha256': hashlib.sha256(ARCHIVE.read_bytes()).hexdigest(),
        'landing_files': {name: hashlib.sha256(content).hexdigest() for name, content in sorted(records.items())},
        'corpus_sha256': version, 'files': files})
